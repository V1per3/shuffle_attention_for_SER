...
def train(SEED, groups,channel,AUG=True):
    setup_seed(SEED)
    MODEL_NAME = 'AUG_sa_area_seed'#.format(groups,channel, SEED)
    data_dir = 'D:/iemotest/iemocap/'
    train_files = []
    train_files2 = []
    valid_files = []
    with open('./IEMOCAP_train_{}.csv'.format(SEED)) as f:
        fr = f.readlines()
        for line in fr:
            train_files.append(data_dir + '/' + line.split('\t')[2])
            if(AUG):
                for i in range(0):
                    train_files2.append(data_dir + '/' + line.split('\t')[2]+'.'+str(i+1))
    with open('./IEMOCAP_dev_{}.csv'.format(SEED)) as f:
        fr = f.readlines()
        for line in fr:
            train_files.append(data_dir + '/' + line.split('\t')[2])
            if(AUG):
   ...
   # train_X_features2 = feature_extractor.get_features(FEATURES_TO_USE, train_X2)
    ...
    model = MODEL.SACNN(groups, channel)
    ...
    for epoch in range(Epochs):
        ...
        print('this epoch: (WA:{},UA:{})\n'.format(correct / len(valid_features_dict), UA))
        logging.warning('this epoch: (WA:{},UA:{})\n'.format(correct / len(valid_features_dict), UA))
        if (ACC > maxACC):
...
if __name__ == '__main__':
    ...
    log_name = 'SA-CNN.log2'#.format(groups, channel)
    logfile = log_name
    ...
    for seed in SEED:
        train(seed, groups, channel,True)
